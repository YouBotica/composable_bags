...
namespace cloud_transform {
  ...
  {
    public:
      CloudTransformerAlt(const rclcpp::NodeOptions & options = rclcpp::NodeOptions()): 
        Node("alt_cloud_transform", options)
      {
        ...
        if (!is_fused){ // We are receiving multiple pointclouds that must be fused
          ...
          sub_front_point_cloud = this->create_subscription<sensor_msgs::msg::PointCloud2>(front_lidar_topic, 10, std::bind(&CloudTransformerAlt::receiveFrontPointCloud, this, std::placeholders::_1));
          sub_left_point_cloud = this->create_subscription<sensor_msgs::msg::PointCloud2>(left_lidar_topic, 10, std::bind(&CloudTransformerAlt::receiveLeftPointCloud, this, std::placeholders::_1));
          sub_right_point_cloud = this->create_subscription<sensor_msgs::msg::PointCloud2>(right_lidar_topic, 10, std::bind(&CloudTransformerAlt::receiveRightPointCloud, this, std::placeholders::_1));
...
RCLCPP_COMPONENTS_REGISTER_NODE(cloud_transform::CloudTransformerAlt)
