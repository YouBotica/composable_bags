    ...
    private:
        void timer_callback()
        {
            if (reader_->has_next()) {
                ...
                RCLCPP_DEBUG(this->get_logger(), "Message timestamp: %d.%d", time_stamp.sec, time_stamp.nanosec);
...
RCLCPP_COMPONENTS_REGISTER_NODE(lidar_components::PlaybackSpoofedNode)
